...
public class SimpleOrderDao implements OrderDao{
    ...
    private static final String SQL_FIND_ALL_ORDERS = "select order_id, s.service_id, s.service_name, s.service_cost,u.user_id, u.user_email, u.user_password, r.role_name , order_is_finished from`clientorder`" +
            " left join service s on `clientorder`.order_service_id = s.service_id" +
            " left join user u on `clientorder`.order_user_id = u.user_id" +
            " left join role r on u.user_role = r.role_id";
    private static final String SQL_FIND_ORDERS_BY_USER_ID = "select order_id, s.service_id, s.service_name, s.service_cost,u.user_id, u.user_email, u.user_password, r.role_name , order_is_finished from `clientorder`" +
            " left join service s on `clientorder`.order_service_id = s.service_id" +
            " left join user u on `clientorder`.order_user_id = u.user_id" +
            " left join role r on u.user_role = r.role_id" +
            " where user_id = ?";
    private static final String SQL_CREATE_ORDER = "insert into `clientorder`(order_service_id, order_user_id,order_is_finished) values(?,?,?)";
    private static final String SQL_DELETE_ORDER = "delete from clientorder where order_id = ?";
